...
public partial class ExfatEditorForm : DarkForm
{
    ...
    public ExfatEditorForm(string imagePath)
    {
        InitializeComponent();
    ...
    private void LoadEntries()
    {
        ...
        using var image = File.OpenRead(_imagePath);
        using var volume = new ExfatVolume(image);
        foreach (ExfatEntry entry in volume.Entries.OrderBy(entry => entry.Path, StringComparer.OrdinalIgnoreCase))
    ...
    private void btnReplace_Click(object? sender, EventArgs e)
    {
        ...
        {
            DarkMessageBox.ShowWarning("Select one file to replace.", "exFAT editor");
            return;
        }
        if (replacementOpenDialog.ShowDialog(this) != DialogResult.OK) return;
        Queue(ExfatEditOperation.Replace(selected.Value.Path, replacementOpenDialog.FileName),
    ...
    private void btnAddFiles_Click(object? sender, EventArgs e)
    {
        ...
        foreach (string source in addFilesOpenDialog.FileNames)
        {
            ...
            Queue(ExfatEditOperation.AddFile(target, source), $"Add file  {target}  ←  {source}");
        }
    }
    ...
    private void btnAddFolder_Click(object? sender, EventArgs e)
    {
        ...
        Queue(ExfatEditOperation.AddDirectoryTree(target, source), $"Add tree  {target}  ←  {source}");
    }
    ...
    private void btnNewDirectory_Click(object? sender, EventArgs e)
    {
        ...
        {
            DarkMessageBox.ShowWarning("Enter the full internal path for the new directory.", "exFAT editor");
            return;
        }
        try
        {
            string target = ExfatImageMaintenance.NormalizeImagePath(txtTarget.Text);
            Queue(ExfatEditOperation.AddDirectory(target), $"New directory  {target}");
        }
    ...
    private void btnDelete_Click(object? sender, EventArgs e)
    {
        ...
        {
            DarkMessageBox.ShowWarning("Select one or more files or directories to delete.", "exFAT editor");
            return;
        }
    ...
    private async void btnApply_Click(object? sender, EventArgs e)
    {
        ...
        try
        {
            var progress = new Progress<FfpfscProgress>(value =>
            {
                ...
                lblStatus.Text = $"{value.Stage} - {FormatBytes(value.BytesProcessed)} / {FormatBytes(value.TotalBytes)}";
            });
            ExfatEditResult result = await ExfatImageMaintenance.ApplyEditsAsync(_imagePath, _operations, progress,
                _cancellation.Token);
            progressEdit.Value = progressEdit.Maximum;
            lblStatus.Text = $"Completed and verified {result.Verification.FileCount:N0} files.";
            string mode = result.Rebuilt ? "Verified transactional rebuild" : "Journaled in-place replacement";
            DarkMessageBox.ShowInformation(
                $"All exFAT changes were applied successfully.\n\n" +
                $"Mode: {mode}\n" +
        ...
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or InvalidDataException or ArgumentException)
        {
            ...
            DarkMessageBox.ShowError(ex.Message, "exFAT edit failed");
        }
        finally
        {
    ...
    private void ExfatEditorForm_FormClosing(object? sender, FormClosingEventArgs e)
    {
        if (_busy)
        {
    ...
    private void Queue(ExfatEditOperation operation, string description)
    {
    ...
    private static string NormalizeOptionalDirectory(string path) =>
        string.IsNullOrWhiteSpace(path) ? string.Empty : ExfatImageMaintenance.NormalizeImagePath(path);
    private static string CombineImagePath(string parent, string name) =>
        ExfatImageMaintenance.NormalizeImagePath(parent.Length == 0 ? name : parent + "/" + name);
