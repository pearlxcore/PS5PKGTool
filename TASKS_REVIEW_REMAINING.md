...
| Interaction | Empty-queue/empty-filter messages added; Ctrl+F now focuses the Tasks search when Tasks is active; only the first queued job switches to Tasks. Still to do: accessible status text surviving row updates; selectable long paths/errors (labels are not selectable); a queued-notification toast with a View action. |
